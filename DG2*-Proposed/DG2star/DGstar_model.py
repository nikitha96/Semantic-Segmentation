...
class Decoder (nn.Module):
    def __init__(self, num_classes):
        ...
        self.layers.append(conv_module(64, 0, 1, 1))
        self.layers.append(conv_module(64, 0, 1, 1))

        self.layers.append(UPS_Block(64,16))
        self.layers.append(conv_module(16, 0, 1, 1))
        self.layers.append(conv_module(16, 0, 1, 1))
...
class Net (nn.Module):
    def __init__(self, num_classes, groups, encoder=None):  #use encoder to pass pretrained encoder
        super().__init__()

        if (encoder == None):
            self.encoder = Encoder(num_classes, groups)
        else:
