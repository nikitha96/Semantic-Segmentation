...
from ModelDataParallel import ModelDataParallel,CriterionDataParallel #https://github.com/pytorch/pytorch/issues/1893
...
class sudNet(nn.Module):
    def __init__(self, num_classes,model):  #use encoder to pass pretrained encoder
        ...
        self.output_conv = nn.ConvTranspose2d( 16, 4, 2, stride=2, padding=0, output_padding=0, bias=True)

    def forward(self, input):
        x = self.encoder(input)
        x = self.decoder(x)
        x1 = self.decoder_last_child(x)
...
def main(args):
    ...
    #Load Model
    #groups = args.groups
    assert os.path.exists(args.dirpath + args.model + ".py"), "Error: model definition not found"
    model_file = importlib.import_module(args.model)
    model = model_file.Net(NUM_CLASSES, args.groups)
    copyfile(args.dirpath + args.model + ".py", savedir + '/' + args.model + ".py")
    ...
    if (not args.state):
        if args.pretrainedEncoder:
            ...
            from DGstar_model_imagenet import Model_Net as ModelNet_imagenet
            pretrainedEnc = torch.nn.DataParallel(ModelNet_imagenet(1000, args.groups))
            pretrainedEnc.load_state_dict(torch.load(args.pretrainedEncoder)['state_dict'])
            pretrainedEnc = next(pretrainedEnc.children()).features.encoder
            #pretrainedEnc = torch.nn.DataParallel(model_file.Net(NUM_CLASSES, encoder=None))
            # for keys in pretrainedEnc.state_dict().keys():
            #     print("keys in pretrained:", keys)
            # pretrainedEnc.load_state_dict(torch.load(args.pretrainedEncoder)['state_dict'])           
        ...
        model = model_file.Net(NUM_CLASSES, args.groups, encoder=pretrainedEnc)  #Add decoder to encoder    
        if args.cuda:
            model = torch.nn.DataParallel(model).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrainedEncoder', default="/home/nikitha/Experiments_pytorch/mobilenetv2/expt6/erfnet_encoder_pretrained.pth.tar")
